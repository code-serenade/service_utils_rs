...
impl WebSocketClient {
    ...
    pub async fn new(url: String) -> Result<Self> {
        ...
        handle_reconnect(&mut client, rx).await;
        // });
        Ok(client)
    }
    ...
    async fn reconnect(&mut self) -> Result<()> {
        ...
        while retries > 0 {
            match connect(&self.url, self.rt.clone()).await {
                Ok(connection) => {
                    self.connection = connection; // 替换当前的 WebSocket 客户端
                    println!("重连成功");
                    return Ok(()); // 成功重连后返回
                }
                Err(e) => {
...
async fn receive_message(mut socket_reader: SocketReader, rt: Sender<String>) -> Result<()> {
    loop {
...
pub async fn connect(url: &str, rt: Sender<String>) -> Result<ClientConnection> {
    ...
    tokio::spawn(receive_message(socket_reader, rt));
    tokio::spawn(handle_send_msg(msg_reciever, socket_writer));
    tokio::spawn(send_ping(msg_sender.clone(), Duration::from_secs(10)));
...
/// 定时发送 Ping 消息
async fn send_ping(msg_sender: super::MsgSender, interval: Duration) -> Result<()> {
...
async fn handle_reconnect(c: &mut WebSocketClient, mut rx: Receiver<String>) {
    while let Some(msg) = rx.recv().await {
        if msg == "reconnect" {
